...
func TestReportSafetyDampened(t *testing.T) {
	t.Run("succeeds for various examples.", func(t *testing.T) {
		...
		testCases := []testCase{
			...
			{
				report:         day2.Report{1, 3, 2, 1, 4},
				expectedResult: false,
			},
			{
				report:         day2.Report{1, 4, 2, 3},
				expectedResult: true,
			},
			{
				report:         day2.Report{9, 6, 7, 8},
				expectedResult: true,
			},
		}
...
func TestCountSafeReports(t *testing.T) {
	t.Run("succeeds for various examples.", func(t *testing.T) {
		...
		for i, testCase := range testCases {
			t.Run(fmt.Sprintf("Test case %d:", i), func(t *testing.T) {
				result := day2.CountSafeReports(testCase.reports, false)
...
func TestCountSafeReportsDampened(t *testing.T) {
	t.Run("succeeds for various examples.", func(t *testing.T) {
		type testCase struct {
			reports        []day2.Report
			expectedResult int
		...
		for i, testCase := range testCases {
			t.Run(fmt.Sprintf("Test case %d:", i), func(t *testing.T) {
				result := day2.CountSafeReports(testCase.reports, true)
